...
export default function App(){const[active,setActive]=useState("Consignments"),[user,setUser]=useState<CurrentUser|null>(null);useEffect(()=>{api<CurrentUser>("/users/me").then(setUser).catch(()=>{})},[]);let page=active==="Inventory"?<InventoryPage/>:active==="Imports"?<ImportsPage/>:active==="Accounts"?<AccountsPage/>:active==="Errors"?<ErrorsPage/>:active==="Consignments"?<ConsignmentsPage/>:active==="Print Queue"?<PrintQueuePage/>:active==="Printers"?<PrintersPage/>:active==="Settings"?<SettingsPage/>:<ShellPage name={active} description={descriptions[active]??"Module foundation."}/>;return <AppLayout active={active} setActive={setActive} user={user}>{page}</AppLayout>}
