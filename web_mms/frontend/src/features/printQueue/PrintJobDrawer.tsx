...
export function PrintJobDrawer({job,onClose,onChanged}:{job:PrintJob;onClose:()=>void;onChanged:()=>void}){
  const[detail,setDetail]=useState<any>(null),[tab,setTab]=useState<Tab>("Overview"),[error,setError]=useState("");
  const[diagnostics,setDiagnostics]=useState<any>(null),[expected,setExpected]=useState(""),[scanned,setScanned]=useState(""),[verified,setVerified]=useState<boolean|null>(null);
  function load(){api(`/print-jobs/${job.id}`).then(setDetail).catch(e=>setError(e.message))}
  useEffect(load,[job.id]);
  useEffect(()=>{if(tab==="Artifact")api(`/print-jobs/${job.id}/diagnostics`).then(setDiagnostics).catch(e=>setError(e.message))},[tab,job.id]);
  ...
  function content(){
    ...
    if(tab==="Artifact")return <pre className="snapshot-view">{JSON.stringify(diagnostics,null,2)}</pre>;
    ...
    return <><dl><div><dt>Transport</dt><dd>{detail.transport_status||"Not started"}</dd></div><div><dt>Spool job</dt><dd>{detail.spool_job_id||"—"}</dd></div><div><dt>Error</dt><dd>{detail.transport_error_code||detail.transport_error_message||"—"}</dd></div></dl>{detail.status==="spooled"&&<button className="primary" onClick={()=>void action("confirm-physical-output")}>Confirm physical output</button>}<h3>Barcode verification</h3><input value={expected} onChange={e=>setExpected(e.target.value)} placeholder="Expected barcode"/><input value={scanned} onChange={e=>setScanned(e.target.value)} placeholder="Scan printed barcode"/><button className="primary" onClick={()=>void verify()}>Verify scan</button>{verified!==null&&<p className={verified?"success-text":"inline-error"}>{verified?"Barcode verified":"Barcode mismatch — block approval"}</p>}</>;
  }
