...
def pairing_code(db: Session = Depends(get_db)):
    code, row = AgentAuthService(db).create_pairing_code(); db.commit()
...
def revoke_agent(agent_id: UUID, db: Session = Depends(get_db)):
    row = db.get(PrintAgent, agent_id)
    if not row: raise HTTPException(404, detail={"code": "PRINT_AGENT_NOT_FOUND"})
    AgentAuthService.revoke(row); db.commit()
    return {"id": str(row.id), "status": row.status}


def _profile(row: PrinterProfile):
    return {"id": str(row.id), "printer_id": str(row.printer_id), "marketplace": row.marketplace.value, "account_id": str(row.account_id) if row.account_id else None,
        ...
        "renderer": row.renderer, "layout_version": row.layout_version, "config": row.config}


@router.get("/printers/{printer_id}/profiles")
def profiles(printer_id: UUID, db: Session = Depends(get_db)):
    return [_profile(row) for row in db.scalars(select(PrinterProfile).where(PrinterProfile.printer_id == printer_id).order_by(PrinterProfile.marketplace)).all()]


@router.post("/printers/{printer_id}/profiles", status_code=201)
def create_profile(printer_id: UUID, payload: PrinterProfileWrite, db: Session = Depends(get_db)):
    ...
    row = PrinterProfile(printer_id=printer.id, **values); db.add(row); printer.is_enabled = True; db.commit(); db.refresh(row)
    return _profile(row)


@router.put("/printer-profiles/{profile_id}")
def update_profile(profile_id: UUID, payload: PrinterProfileWrite, db: Session = Depends(get_db)):
    ...
    db.add(AuditEvent(entity_type="printer_profile", entity_id=str(row.id), action="updated", changes={"renderer":payload.renderer,"marketplace":payload.marketplace,"layout_version":row.layout_version}))
    db.commit(); return _profile(row)


@router.get("/printer-profiles/{profile_id}/approvals")
def approvals(profile_id: UUID, db: Session = Depends(get_db)):
...
def approve(profile_id: UUID, payload: RendererApprovalWrite, db: Session = Depends(get_db)):
    ...
    try: row = RendererApprovalService(db).approve(profile, job, format_key=payload.format_key, notes=payload.notes); db.commit(); db.refresh(row)
...
def revoke_approval(approval_id: UUID, db: Session = Depends(get_db)):
    row = db.get(RendererProfileApproval, approval_id)
    if not row: raise HTTPException(404, detail={"code": "APPROVAL_NOT_FOUND"})
    RendererApprovalService(db).revoke(row); db.commit(); return {"id": str(row.id), "revoked": True}
